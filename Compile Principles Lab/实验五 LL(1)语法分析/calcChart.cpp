...
*/
map<GrammarItem, set<GrammarItem>> first;
...
map<GrammarItem, set<GrammarItem>> follow;
void calcFollow()
{
        ...
        for (auto product : products) // 遍历非终结符的每个产生式
        {
            ...
            if (!alpha.isTerminal)//非终结符看first集合
            {
                ...
                if (first[alpha].count(GrammarItem(epsilon, true)))
                {
                    ...
                    {
                        table[item][b] = product;
                    }

                    if (follow[item].count(GrammarItem("$", true)))
                    {
                        table[item][GrammarItem("$", true)] = product;
                    }
                }
            }
            //终结符
...
vector<vector<GrammarItem>> predictPrase()
{
    ...
    stk.push(GrammarItem("$", true));
    stk.push(GrammarItem("E", false));
    string str;
    cin >> str;
...
void draw(vector<vector<GrammarItem>>result)
{
    ...
    map<GrammarItem, int> cnt;//在点的字符串里添加的空格数
    ...
    for(auto production:result)
    {
        ...
        auto xx=cnt[production[0]];
        for(int i=0;i<xx/2;i++)
        {
            head=" "+head;
        }
        for(int i=0;i<xx-xx/2;i++)
        {
            head=head+" ";
        }
       // cnt[production[0]]++;
