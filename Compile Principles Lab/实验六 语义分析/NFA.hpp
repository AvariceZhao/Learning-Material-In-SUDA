...
public:
	...
	static NFA* buildNFAFromFile(const char* file)
	{
		ifstream in(file);
		map<string, vector<string>> tempTree;
		int lines = 0;
		...
		while (in >> father >> son)
		{
			if (root.size() == 0)
				root = father;
			tempTree[father].push_back(son);

		}
		return new NFA(tempTree, root);
	...
	{
		auto NFAStart = new NFAState(++NFAStateCount), NFAEnd = new NFAState(++NFAStateCount);
		NFA* ans = new NFA();
